...
import cv.dge.dge_api_intermed_lab.application.emprego.dto.RelatorioAcompanhamentoVinculo;
...
public class GestaoRelatorioAcompanhamentoRepository {
    ...
    public List<RelatorioAcompanhamentoEstagiarioSelectResponse> listarEstagiarios(Integer entidadeId) {
        String sql = """
                ...
                WHERE c.entidade_id = ?
                  ...
                  AND UPPER(COALESCE(c.tipo_oferta, '')) = 'OFERTA_ESTAGIO'
                  AND UPPER(COALESCE(c.estado, 'A')) IN ('A', 'ATIVO')
                ORDER BY c.nome, c.pessoa_id
                """;
        return jdbcTemplate.query(sql, (rs, rowNum) -> new RelatorioAcompanhamentoEstagiarioSelectResponse(
                getLong(rs, "pessoa_id"),
                rs.getString("nome")
        ), entidadeId);
    }
